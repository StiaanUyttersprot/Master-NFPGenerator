...
public class ComplexPolygonStage {
	...
	static double sceneSizeX = 1000;
	static double sceneSizeY = 1000;
...
//	}
	...
	private static Stage drawComplexPolygon(List<Edge> complexPolygon) {
		...
		for(Edge e: complexPolygon){
			if(e.getStartPoint().getxCoord()>biggestXCoordValue){
				biggestXCoordValue = e.getStartPoint().getxCoord();
			}
			if(e.getEndPoint().getxCoord()>biggestXCoordValue){
				biggestXCoordValue = e.getEndPoint().getxCoord();
			}
			if(e.getStartPoint().getyCoord()>biggestYCoordValue){
				biggestYCoordValue = e.getStartPoint().getyCoord();
			}
			if(e.getEndPoint().getyCoord()>biggestYCoordValue){
				biggestYCoordValue = e.getEndPoint().getyCoord();
			}
		}
