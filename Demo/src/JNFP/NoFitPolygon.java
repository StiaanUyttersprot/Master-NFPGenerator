...
public class NoFitPolygon {
	...
	// this method will remove coordinates that aren't necessary to draw the
	// nfp(when more than two points fall on the same line
		...
		for (List<Coordinate> coordinateList : nfpPolygonsList) {
			...
			if (coordinateList.size() > 1) {
				while (start + 1 < coordinateList.size()) {
					...
					while (coordinateList.size() > 1 && start + 1 < coordinateList.size() && coordinateList
							.get(checkPoint).dFunctionCheck(coordinateList.get(start), coordinateList.get(start + 1))) {
						coordinateList.remove(start + 1);
						if (checkPoint >= coordinateList.size())
							checkPoint = 0;
					}
					start++;
				}
	...
	protected boolean containsPoint(Coordinate coordinate) {
		...
		for (List<Coordinate> partList : nfpPolygonsList) {
			for (int i = 0; i < partList.size(); i++) {
				...
				if (testEdge.containsPoint(coordinate))
					return true;
				if (partList.get(i).equals(coordinate))
					return true;
			}
		}
		return false;
	...
	public Polygon[] toPolygonList(double xSize, double ySize, double sizeFactor) {
		...
		for (int i = 0; i < nfpPolygonsList.size(); i++) {
			...
			for (Coordinate coord : nfpPolygonsList.get(i)) {
				...
				polygonList[i].getPoints().add(sizeFactor * coord.getyCoord() + ySize / 2);
			}
		}
	...
	public Polygon[] toPolygonList(double xSize, double ySize) {

		Polygon[] polygonList = new Polygon[nfpPolygonsList.size()];
		for (int i = 0; i < nfpPolygonsList.size(); i++) {
			polygonList[i] = new Polygon();
			for (Coordinate coord : nfpPolygonsList.get(i)) {
				polygonList[i].getPoints().add(coord.getxCoord());
				// yCoord*-1 to invert to normal axis
				polygonList[i].getPoints().add(coord.getyCoord());
			}
		}
