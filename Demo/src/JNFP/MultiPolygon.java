...
/**
 * this class contains a polygon to be used to generate the no-fit polygon. The
 * polygon exists of coordinates and can have holes in it.
 * 
...
public class MultiPolygon {
	...
	public MultiPolygon(File file) throws FileNotFoundException {
		...
		try {
			...
			// this array contains the same information but in pairs of
			// coordinates
	...
	protected Polygon makeOuterPolygon(double xSize, double ySize, double sizeFactor) {
		...
		for (Coordinate coord : outerPolygon) {
			...
			polygon.getPoints().add( sizeFactor * coord.getyCoord() + ySize / 2);
		}
	...
	protected Polygon[] makeHoles(double xSize, double ySize, double sizeFactor) {
		...
		for (int i = 0; i < nHoles; i++) {
			...
			for (Coordinate coord : holes[i]) {
				...
				polyHoles[i].getPoints().add(sizeFactor * coord.getyCoord() + ySize / 2);
			}
		}
		return polyHoles;
	}
	...
	/**
	 * translate the multipolygon using the vector described by it's x- and
	 * y-coord.
	 * 
	 * @param x
	 *            the x-value of the translationvector
	 * @param y
	 *            the y-value of the translationvector
	 */
	public void translate(double x, double y) {
		...
		// for the edges the new minimum and maximum values need to be
		// recalculated
	...
	public boolean checkClockwise(Coordinate[] polygon) {
		...
		// If the result is positive the curve is clockwise, if it's negative
		// the curve is counter-clockwise.
		for (int i = 0; i < polygon.length; i++) {
			if (i < polygon.length - 1) {
				// Sum over the edges, (x2-x1)(y2+y1). If the result is positive
				// the curve is clockwise, if it's negative the curve is
				// counter-clockwise.
	...
	protected Edge findUntraversedEdge() {
		...
		while (untraversedEdge == null && i < outerPolygonEdges.length) {
			if (!outerPolygonEdges[i].isTraversed())
				untraversedEdge = outerPolygonEdges[i];
			i++;
		...
		while (untraversedEdge == null && i < holeEdges.length) {
			...
			while (untraversedEdge == null && j < holeEdges[i].length) {
				if (!holeEdges[i][j].isTraversed())
					untraversedEdge = holeEdges[i][j];
				j++;
	...
	protected Coordinate searchStartPoint(Edge possibleStartEdge, MultiPolygon orbPoly) {
		...
		// for every point of the orbiting polygon that can be placed at that
		// spot
		int orbPointIndex = 0;
		...
		for (int i = 0; i < orbPoly.getOuterPolygon().length; i++) {
			...
			if (!polygonsIntersectPointInPolygon(this, orbPoly)) {
				...
				if (orbPointIndex == 0) {
					startPointPossible = possibleStartEdge.edgesOrientatedRight(
							orbPoly.getOuterPolygonEdges()[orbPoly.getOuterPolygonEdges().length - 1],
							orbPoly.getOuterPolygonEdges()[orbPointIndex]);
				} else {
					startPointPossible = possibleStartEdge.edgesOrientatedRight(
							orbPoly.getOuterPolygonEdges()[orbPointIndex - 1],
							orbPoly.getOuterPolygonEdges()[orbPointIndex]);
				}
			}
			// keep looking for a place where the polygons don't overlap, till
			// the end of the line has been reached
			if (startPointPossible) {
				// trim the vector made by the possible startEdge and the
				// current start point
				while (!currentStartPoint.equals(possibleStartEdge.getEndPoint())
						&& polygonsIntersectPointInPolygon(this, orbPoly)) {
				...
				if (currentStartPoint.equals(possibleStartEdge.getEndPoint())
						&& !polygonsIntersectPointInPolygon(this, orbPoly)) {
					return currentStartPoint;
	...
	protected Coordinate searchOrbStartPoint(Edge possibleStartOrbEdge, MultiPolygon orbPoly) {
		...
		for (int i = 0; i < getOuterPolygon().length; i++) {
			...
			if (!polygonsIntersectPointInPolygon(this, orbPoly)) {
				...
				if (statPointIndex == 0) {
					startPointPossible = possibleStartOrbEdge.edgesOrientatedRight(
							getOuterPolygonEdges()[getOuterPolygonEdges().length - 1],
							getOuterPolygonEdges()[statPointIndex]);
				} else {
					startPointPossible = possibleStartOrbEdge.edgesOrientatedRight(
							getOuterPolygonEdges()[statPointIndex - 1], getOuterPolygonEdges()[statPointIndex]);
				}
				if (startPointPossible) {
					// trim the vector made by the possible startEdge and the
					// current start point
					while (!currentStartPoint.equals(possibleStartOrbEdge.getEndPoint())
							&& polygonsIntersectPointInPolygon(this, orbPoly)) {
					...
					if (currentStartPoint.equals(possibleStartOrbEdge.getEndPoint())
							&& !polygonsIntersectPointInPolygon(this, orbPoly)) {
						return currentStartPoint;
			...
			if (startPointPossible) {
				...
				do {
					...
					currentStartPoint.translate(nextPossibleSpotVector);
						...
						&& (polygonsIntersectPointInPolygon(this, orbPoly) || nfp.containsPoint(currentStartPoint)));

				if (currentStartPoint.equals(possibleStartEdge.getEndPoint())
						&& !polygonsIntersectPointInPolygon(this, orbPoly) && !nfp.containsPoint(currentStartPoint)) {
					return currentStartPoint;
				} else if (!currentStartPoint.equals(possibleStartEdge.getEndPoint())
						&& !nfp.containsPoint(currentStartPoint)) {
					return currentStartPoint;
				}
			}
			orbPointIndex++;
	...
	protected static boolean polygonsIntersectPointInPolygon(MultiPolygon statPoly, MultiPolygon orbPoly) {
		...
		if (polygonsIntersectEdgeIntersect(statPoly, orbPoly))
			return true;
		if (polygonsIntersectEdgeOverlap(statPoly, orbPoly))
			return true;
		...
		for (Edge e : statPoly.getOuterPolygonEdges()) {
			for (Edge f : orbPoly.getOuterPolygonEdges()) {
				if (Math.abs(e.getAngle() - f.getAngle()) % (Math.PI * 2) == 0) {
					if (!e.getStartPoint().equalValuesRounded(f.getEndPoint())
							&& !e.getEndPoint().equalValuesRounded(f.getStartPoint())) {
						...
						if (f.containsPoint(e.getStartPoint()) || f.containsPoint(e.getEndPoint()))
							return true;
					}
				}
			}
	 ...
	 * @param coord
	 *            the coord that may be in the polygon
			...
			for (Coordinate[] hole : holes) {
				...
				for (i = 0; i < polyCorners; i++) {
					if ((hole[i].getyCoord() < coord.getyCoord() && hole[j].getyCoord() >= coord.getyCoord()
							|| hole[j].getyCoord() < coord.getyCoord() && hole[i].getyCoord() >= coord.getyCoord())
							&& (hole[i].getxCoord() <= coord.getxCoord() || hole[j].getxCoord() <= coord.getxCoord())) {
						// ^= is bitwise XOR assignement
	...
	protected void calcDeltaAngles() {
		...
		for (int i = 0; i < outerPolygonEdges.length; i++) {
			if (i > 0) {
				outerPolygonEdges[i]
						.setDeltaAngle(outerPolygonEdges[i].getEdgeAngle() - outerPolygonEdges[i - 1].getEdgeAngle());
			} else {
				outerPolygonEdges[i].setDeltaAngle(outerPolygonEdges[i].getEdgeAngle()
						- outerPolygonEdges[outerPolygonEdges.length - 1].getEdgeAngle());
			}
		...
		for (int i = 0; i < outerPolygonEdges.length; i++) {
			if (i > 0) {
				if (Math.signum(outerPolygonEdges[i].getDeltaAngle()) != Math
						.signum(outerPolygonEdges[i - 1].getDeltaAngle())) {
					outerPolygonEdges[i].setTurningPoint(true);
				}
			} else {
				if (Math.signum(outerPolygonEdges[i].getDeltaAngle()) != Math
						.signum(outerPolygonEdges[outerPolygonEdges.length - 1].getDeltaAngle())) {
					outerPolygonEdges[i].setTurningPoint(true);
				}
			}
		}
	...
	// the standard algorithm calculates the edge angles in the way the edge is
	// oriented, we need the angle in counterclockwise direction for
	 ...
	 * 
	 * @param polyB
	 *            the polygon that may overlap this one
	...
	public boolean overlapping(MultiPolygon polyB) {
		...
		if (polygonsIntersectEdgeOverlap(polyB, this))
			return true;
		while (f < polyB.getOuterPolygonEdges().length && !overlap) {
			...
			while (k < getOuterPolygonEdges().length && !overlap) {
				...
				if (edgeA.containsPoint(edgeB.getStartPoint())
						&& !edgeB.getEndPoint().dFunctionCheck(edgeA.getStartPoint(), edgeA.getEndPoint())) {
					if (!edgeA.getStartPoint().equalValuesRounded(edgeB.getStartPoint())
							&& !edgeA.getStartPoint().equalValuesRounded(edgeB.getEndPoint())) {
						if (!edgeA.getEndPoint().equalValuesRounded(edgeB.getStartPoint())
								&& !edgeA.getEndPoint().equalValuesRounded(edgeB.getEndPoint())) {
							if (edgeB.getEndPoint().dFunction(edgeA) > 0)
								overlap = true;
						}
					}
				}
